...
**Versão CASA:** 2.0 · **Status:** accepted (2026-07-31) · **Mantenedor:** atplus-digital/maicon
...
`scripts/docs-check` roda no gate automatizado de todo repo T1 (§3 — CI ou pre-commit hook). **Erro = exit 1 por padrão**. A janela de adoção usa `--adoption` com baseline versionada e monotônica; `--warn-only` é alias depreciado com a mesma semântica segura — nenhum dos dois é bypass incondicional (§10, ADR-0017).
...
9. **Ratchet da adoção**: `--adoption --base-ref <ref>` exige `docs/.docs-check-baseline.json` canônico. O multiconjunto atual precisa ser idêntico ao teto; erro novo falha, erro resolvido pede `--emit-baseline`, zero exige remover baseline/modo, e a baseline do PR só pode diminuir em relação à base Git. A primeira captura só é autorizada contra base CASA 1.x e não pode conter erro ausente do snapshot dessa base, revalidado pela toolchain nova; paths e OIDs da materialização precisam coincidir com `git ls-tree`, e `export-ignore`, `export-subst`, filtros, symlink ou submódulo falham fechados. Base 2.x sem baseline já encerrou a janela e não pode reabri-la. `--warn-only` é alias depreciado (ADR-0017 / SPEC-0009).
...
Repo **existente**: o `casa-init` instala a base e o `docs-check` relata o que falta; a migração de **conteúdo** exige julgamento e segue incremental, nesta ordem: (1) **limpar a árvore** — os duplicados saem antes de qualquer outra coisa, é o gate mais barato e de maior efeito; (2) preencher o `AGENTS.md` router (pode parar aqui em T0); (3) declarar o tier; (4) extrair capítulos de contexto do que hoje está espalhado (convenções do README de specs → `CONVENTIONS.md`); (5) frontmatter mínimo nos docs existentes — doc tocado, doc migrado; status legado migra pelo estado **real verificado** (spec só vira `implemented` se a implementação está no ar), nunca por rename mecânico; template antigo em pasta de docs vai para `docs/templates/`; referência a conceito fora do padrão ou vira campo registrado (ADR do repo, §6) ou é removida; ADRs já emendados inline ganham `VERDADE ATUAL` apontando para a verdade, sem reescrever histórico; (6) se o repo ainda não passa limpo, numa branch sobre a base de integração rode `python3 scripts/docs-check --adoption --emit-baseline --base-ref origin/main`, commite `docs/.docs-check-baseline.json` e use no gate `python3 scripts/docs-check --adoption --base-ref origin/main`; cada melhoria exige emitir/commitar o teto menor; ao chegar a zero, remova baseline e flag e volte ao comando estrito.

**Migração incompatível 1.x → 2.0 (janela de adoção):** repo que já usa `--warn-only` deve atualizar a toolchain com `casa-init`, criar a primeira baseline pelo comando acima no mesmo PR e substituir a flag por `--adoption`. Até esse PR, mantenha a toolchain 1.x pinada; depois dele, erro novo deixa de ser anistiado. Na captura inicial, a toolchain 2.0 arquiva e revalida o snapshot do ref para provar que cada erro congelado já existia na base 1.x. Host sem `origin/main` usa um ref Git verificável equivalente (no pre-commit local, `--base-ref HEAD`). No CI, o ref precisa apontar para a branch de integração protegida e o workflow também precisa de revisão protegida: como qualquer gate versionado, editar o próprio comando pode contorná-lo. `casa-version` muda para `2.0` no mesmo PR que instala a baseline/gate; removida a baseline ao chegar a zero, a janela não pode ser reaberta (ADR-0010).
