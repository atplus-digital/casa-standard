...
calculado do estado real (arquivos + ledger), arquivo nascido do template com data e H1
certos, reserva registrada — eliminando o erro manual que gera colisão e renumeração
(issue #12).
...
2. Coleta números usados: arquivos `NNNN-*.md` da camada + rows da seção correspondente do
...
4. Cria `docs/<camada>/NNNN-slug.md` do template do repo: `date` de hoje, H1 = título,
   status default do template (`proposed`/`draft`).
...
- Nunca sobrescreve arquivo; exit ≠ 0 só por erro de uso (camada inválida, título sem slug,
  template ausente, backlog no lugar errado).
...
| 2 | reservas nas duas camadas | numerar cada camada de forma independente e sequencial |
