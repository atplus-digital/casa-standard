# Backlog — casa-standard

Pendências e reservas de numeração (STANDARD §5, ADR-0002). Conteúdo livre — não é camada
validada pelo `docs-check`.

## Decisões pendentes

(nenhuma no momento)

## Pendências externas


## Reservas de NNNN

ADRs:
- 0001–0003: usados (aprendizados do piloto, 2026-06-09).
- 0004: usado (bootstrap `casa-init`, 2026-06-09).
- 0005: usado (distribuição via `install.sh` curl|sh, 2026-06-09).
- 0006: usado (gate por host do remote, 2026-06-12).
- 0007: usado (governança versionada e invariantes do core, 2026-06-13).
- 0008: usado (registry fechado de capacidades reconhecidas, 2026-06-20).
- 0009: usado (ponte CLAUDE.md para hosts sem AGENTS.md, 2026-07-05).
- 0010: usado (casa-version é promessa do repo, 2026-07-05).
- 0011: usado (aviso de backlog em docs/context/, 2026-07-05).
- 0012: usado (DoD exercita casos de borda; incremento divide ou corta, 2026-07-05).
- Próximo livre: 0013 — registre aqui a reserva ao abrir o PR que o usa.

Specs:
- 0001: usado (`casa-init`, 2026-06-09).
- 0002: usado (`install.sh`, 2026-06-09).
- 0003: usado (`docs-check` invariantes core, 2026-06-13).
- 0004: usado (capacidade TESTS reconhecida pelo `docs-check`, 2026-06-20).
- 0005: usado (ponte CLAUDE.md no bootstrap, 2026-07-05).
- 0006: usado (sinalização de manutenção no docs-check, 2026-07-05).
- Próximo livre: 0007.
